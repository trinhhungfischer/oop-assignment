...
import hust.soict.khmt.aims.utils.MemoryDaemon;
...
public class Aims {
		...
		while (status) {
			try {
				...
				if (status != false) {
					...
					System.out.println("________________________________________________________________________");
					if (currentOrder > 0) {
						System.out.println("_______________________________Order " + currentOrder
	...
	public static void addItem() {
		...
		if (keyboard.hasNextLine()) {
			...
			if (listMedia.get(k - 1).getClass() == DigitalVideoDisc.class
					|| listMedia.get(k - 1).getClass() == CompactDiscs.class) {
				System.out.println("Do you wana play this DVD: 1.Yes 2.No");
				int isPlay = keyboard.nextInt();
				if (isPlay == 1) {
